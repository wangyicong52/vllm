...
class DeepseekSparseSWAMetadataBuilder(AttentionMetadataBuilder):
    ...
    def build(
        ...
        # DSpark drafts a non-causal query block: every query token attends to
        # the trailing context window PLUS all query tokens (including future
        # ones). The scheduler flags this by clearing ``causal`` on the common
        # metadata. All other paths (target decode, MTP) stay causal and use the
        # standard window-topk kernel. Prefill SWA indices are built inside
        # ``_build_deepseek_v4_metadata`` on this fork, so ``build`` only handles
        # the decode segment here.
