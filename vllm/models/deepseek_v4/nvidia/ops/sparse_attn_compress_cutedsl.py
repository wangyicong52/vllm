...
def store_compressed_kv_cutedsl(
    ...
    """Run ONLY the C128 norm+RoPE+store kernel over a precomputed compressed_kv.

    Reuses the same store kernels and store-target dispatch as
    ``split_kv_compress_norm_rope_insert_sparse_attn_cutedsl`` (legacy UE8M0
    paged uint8, FlashInfer full bf16, FlashInfer full fp8). The compression /
    aggregation step is owned by the C128 online merge kernel and is NOT run
    here; ``compressed_kv`` is supplied directly.
    """
    if k_cache.ndim != 3:
        raise ValueError(
            "CuTe DSL sparse-attn store expects the real DeepSeek V4 "
